import { Dispatch } from 'react'
import Eth from 'ethjs'
import erc720 from './erc720.json'
import { getAccounts, getNetwork } from '../../../ethrpc'
import { addToken, addTokenData } from '../reducers/tokens'
import { getTokens } from '../../../config/getConfig'
import { saveToLocalStorage, getValueFromLocalStorage } from '../../../features/localStorage'

export const getTokenList = (provider: any) => (dispatch: Dispatch<any>) =>
  getNetwork(provider)
    .then((chainId: string) => {
      const tokens = getTokens(parseInt(chainId))
      const localTokens = getValueFromLocalStorage(chainId, 'TOKEN')
      const all = tokens.concat(localTokens)
      Array.isArray(all) && all.map((address: string) => {
        dispatch(getToken(provider, address))
      })
    })

export const getToken = (provider: any, address: string, onComplete?: any) => (dispatch: Dispatch<any>) => {
  const eth = new Eth(provider)
  const token = eth.contract(erc720).at(address)

  eth.getCode(address)
    .then((result: string) => {
      if (result === '0x0') {
        return onComplete ? onComplete(new Error('Not a contract')) : null
      }

      dispatch(addToken({ address }))

      token.symbol().then((symbol: string[]) => symbol[0])
        .then((symbol: string) => dispatch(addTokenData({ data: { address, symbol } }))).catch()

      token.name().then((name: string) => name[0])
        .then((name: string) => dispatch(addTokenData({ data: { address, name } }))).catch()

      getAccounts(provider).then((accounts: string[]) => {
        token.decimals().then((decimals: any) => {
          token.balanceOf(accounts[0])
            .then((balance: any) => {
              dispatch(addTokenData(
                { data: { address, balance: balance[0].toString() / Math.pow(10, decimals[0].toNumber()) } }
              ))
              onComplete && onComplete(true)
            })
        })
      })
    })
}

export const addCustomToken = (provider: any, address: string) => (dispatch: Dispatch<any>) =>
  new Promise((resolve, reject) => {
    const onComplete = (response: any) =>
      response instanceof Error ? reject(response)
        : getNetwork(provider)
          .then((chainId: string) => {
            saveToLocalStorage(chainId, 'TOKEN', address)
            resolve()
          })

    dispatch(getToken(provider, address, onComplete))
  })
